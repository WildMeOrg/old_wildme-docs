...
This list can be configured to be a smaller subset of countries. Reach out to Wildbook support for configuration assistance.

Set Location ID: Select from a list of locations where the encounter occurred. Once you've selected the appropriate location, click Set Location ID to confirm the new value.
...
Labeled Keyword: Labelled keywords are provided to allow admins to preset values for users to apply to a MediaAsset. To select a labeled keyword:

Hover on the add new keyword button or hover over an existing keyword.
Under add new labeled keyword, select select label.
From the dropdown, select the desired label. If the list collapses, the selection remains selected.
Select select value.
...
Labeled keywords are set in the CommonConfiguration.properties file; reach out to your admin or to Wildbook support for configuration assistance.

Keyword: Keywords allow for easy assignment of any string to a MediaAsset.

To select an existing keyword:

Hover on the add new keyword button or hover over an existing keyword.
Under add new keyword, select select keyword.
From the dropdown, select the desired keyword.
...
Hover on the add new keyword button or hover over an existing keyword.
Under add new keyword, type in the text field with the helper text "or enter new".
Hit enter to confirm the keyword.

Once entered, the keyword appears on the image.

Keywords are not automatically curated or checked against existing keywords, so you can create duplicate keywords easily. Admins can manage keywords from Administer > Photo Keywords.

Remove this image: If an image is low quality or not providing value, select this button to remove the image from the encounter. This does not remove the image from the database and can be reversed.

Match results: After an annotation has run through ID, you can select this button to see the proposed matches for the annotation. This will take to you to the results page, where you will actually perform matching.

Start another match: If the annotation has no viable matches, or if there have been extension additions to the database, you may want to run the annotation back through the ID process. Select this button to start the process again.

Add annotation: (Alpha) If detection failed to recognize an animal, you can click this option to access the manual annotation screen.

Add image to Encounter: If there were images that did not get added with the initial upload, you can click this button to browse to the images and upload them to the system. Note that detection is not run on these images automatically. Reach out to Wildbook support for assistance.
...
New individual: If you've determined the encounter is of an individual that does not exist in the current Wildbook, you should enter a unique ID. This ID needs to be distinct from any Individual ID that currently exists. When you've entered the ID, click New to confirm the value.

If have determined the encounter is related to an existing individual, fill out the information under Add to existing individual ID.

Existing individual: Begin typing the name or species associated with the individual you want to associate the encounter with. A dropdown will display a list of all matching individuals. Select from the list.
...
Taxonomy: Using the dropdown, select from the available names. When you've selected the correct taxonomy, click Set to confirm the value.
...
Group Role: Using the text field, provide a description of the role the observed animal has within their group. When you've entered an appropriate description of the role, click Submit Edit to confirm the value.
...
If the animal is a Marked Individual, you can use the social role and relationship functionality to denote this information.
...
Life stage is configured in the commonConfiguration.properties file. To request the life stage field be added or to request additional values, contact Wildbook support for assistance.
...
Auto_sourced: an encounter that was pulled from youtube.
...
Measurements are used to track numeric information associated with an encounter. These are set in the commonConfiguration.properties file. To request measurements be added, contact Wildbook support for assistance.
...
Statellite Tag Metadata provides information for information associated with tagging done using satellites, including name of satellite, serial number, and argos PTT. Click Set after the tag information is entered.
...
Open the appropriate analysis, then click Set to confirm the information.
