...
11. Roll over each potential match (ranked 1-12 from most likely to least likely match) to inspect what each algorithm though was similar between the Annotations. Each result includes a rank, an algorithm-specific score, and potentially additional ID and date metadata to help you evaluate the result. [See Identification for more information about each algorithm used in Wildbook.](ia_pipeline.d#identification) 
